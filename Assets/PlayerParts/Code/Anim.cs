using UnityEngine;

public static class Anim
{
    public const string  Idle = "Idle"; 
    public const string  Run_Start = "Run_Start";
    public const string  Run = "Running";
    public const string  Run_Stop = "Run_Stop";
    public const string  Rise = "Rise";
    public const string  Peak = "Peak";
    public const string  Fall = "Fall";
    public const string Turn = "Run_Turn";
    public const string Land = "Land";
    public const string Roll = "Roll";
    public const string Dash_Start = "Dash_Start";
    public const string Dashing = "Dashing";
    public const string Dash_Exit = "Dash_End";
    public const string Rest_Start = "Rest_Start";
    public const string Resting = "Resting";
    public const string Sleep_Start = "SleepStart";
    public const string Sleeping = "Sleeping";
    public const string GroundAttack_1 = "LightAtk";
    public const string GroundAttack_2 = "UpLightAtk";

}
